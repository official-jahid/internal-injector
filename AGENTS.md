...
| 2026-09-17 | Bug found and fixed: `[uint]` accelerator is invalid in Windows PowerShell 5.1 (it is the *name* of the type in PS 7), so `WriteProcessMemory` threw under `irm \| iex`; replaced with `[uint32]` everywhere. Validated by a live elevated injection (`exit 0`, module verified loaded) |
| 2026-09-17 | Bug found and fixed: the remote DLL-path buffer was freed in a `finally` block even when `WaitForSingleObject` had timed out, so a still-running remote `LoadLibraryW` read freed memory and the game died on slower machines. Buffer is now freed only after the load returns; remote wait raised 10s -> 60s |
| 2026-09-17 | Injection is now verified instead of assumed: `EnumProcessModulesEx`/`GetModuleFileNameExW` confirm `REGIX.dll` really is in the target, plus a 20s liveness watch. A timed-out load is never reported as success; `GetExitCodeProcess` is used to report the target's exit code |
| 2026-09-17 | Root cause of "injection kills HD-Player" proven by experiment: REGIX.dll into `notepad` = survived 60s; benign `version.dll` from `%TEMP%` into `HD-Player` = survived 60s; REGIX.dll into a *renamed* `powershell.exe` called `HD-Player.exe` = terminated itself after ~62s with exit code 0. The DLL performs its own process checks after loading (`TerminateProcess`, `IsDebuggerPresent`, `QueryFullProcessImageNameW`, emulator-target strings) and contains an unknown blob + license gate. The injector never terminates the target; the script now reports this honestly and exits 1 instead of printing a fake success |
| 2026-09-17 | `injector.exe` in the repo folder is a disguised payload: version resource impersonates Microsoft `SearchProtocolHost.exe`, Authenticode = HashMismatch, embedded Python runtime strings. It must not be committed, distributed, or auto-run - the chain stays script-only (documented in README) |
| 2026-09-17 | Liveness watch is user-tunable: `-VerifySeconds` (default 30, minimum 5) replaces the hard-coded 20s watch; wording of the DLL-side-termination error cites the experimental proof so users are not told the injector failed |
| 2026-09-17 | Live elevated end-to-end run after the fixes: fresh 26.1 MB download -> verified module load in HD-Player -> target alive 20s (reported) and still alive 60s+ afterwards, `exit 0` |
